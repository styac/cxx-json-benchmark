...
static char* ReadJSON(const std::filesystem::path p, size_t* length) {
    FILE *fp = fopen(p.string().data(), "rb");
    return ReadJSON(fp, length);
}
