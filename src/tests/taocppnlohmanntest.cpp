...
public:
    ...
    virtual ParseResultBase* Parse(const char* j, size_t length) const {
        ...
        try {
            ...
            tao::json::from_string(j, length, handler);
    ...
    virtual StringResultBase* Stringify(const ParseResultBase* parseResult) const {
        ...
        //tao::json::to_stream(oss);
        tao::json::nlohmann::from_value(sr,pr->root);
        //sr->s = oss.str();
        return sr;
    }
#endif
...
//        sr->s = oss.str();
...
#if TEST_CONFORMANCE
    virtual bool ParseDouble(const char* j, size_t jsize, double* d) const override {
        try {
    ...
    virtual bool ParseString(const char* j, size_t jsize, std::string& s) const override {
        try {
            json root = json::parse(j);
